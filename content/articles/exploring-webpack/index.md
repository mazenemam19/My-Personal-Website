...
![](https://cdn-images-1.medium.com/max/303/1*JX5rAwXLl6DTXu9c62q2DA.png)
...
Webpack also helps us develop projects by running a development server. Back in the old days, to test your code changes, you would need to save the changed files and hit the refresh button, but this isn't the case now, you are no longer clicking the refresh button. When you run the start script in package.json, the script listed under start runs that creates a new instance of the WebpackDevServer, passing in the compiler and configuration. A listener method on the instance is called, passing in the port and host values. Then it clears the console and reads " Starting the development server…". The browser is opened with the correct dev URL. Lastly, two listeners are added for when the process is killed, which turns off the web server, and exits the start.js process.  
...
Have you noticed that a typical React app doesn't have Webpack in package.jsonor a webpack.config.js file in the project tree?

![](https://cdn-images-1.medium.com/max/148/1*mmG58A1Qgj5Wi07HZhrK0A.png)

![](https://cdn-images-1.medium.com/max/574/1*unfr0Aq_NFnQEaZBwa61jQ.png)
...
![](https://cdn-images-1.medium.com/max/873/1*gWOsgcNiOVLTH44_zyG2Fw.png)

![](https://cdn-images-1.medium.com/max/871/1*1MtSBS7P-jTbQD8IdPgxSg.png)

![](https://cdn-images-1.medium.com/max/965/1*nvP5JcmUqbcOMjPLeduWlQ.png)
...
In this file, we are using BundleAnalyzerPlugin to open up a server after the build to visually display the size and contents of the Webpack output files, and using ProgressBarPluginto show a progress bar in the terminal during the build, indicating how much of the build has been completed and how long it has been running. Lastly, we are using chalk to add some coloring to the terminal

This is the result of running our analysis script:

![](https://cdn-images-1.medium.com/max/1024/1*uYpNYAP4tO7Zuw4V5A61lQ.png)

![](https://cdn-images-1.medium.com/max/1024/1*PyoIkXOBO5eeamelo5RhVg.png)

![](https://cdn-images-1.medium.com/max/1024/1*En87r3iwX7UzU7A10KP1VQ.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*iBbjNjdiLjmFKkq4z40nJA.png)

![](https://cdn-images-1.medium.com/max/779/1*wEdvAUS6_s563OmFxaN8DA.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*yPuXt_jBVD69l1Kcd6heRw.png)
...
![](https://cdn-images-1.medium.com/max/928/1*F5RzoIdmOwoQogRTaSKeFw.png)

![](https://cdn-images-1.medium.com/max/1024/1*lv5Lc0XeTL9WeI2-VyekWg.png)

![](https://cdn-images-1.medium.com/max/1024/1*QQdBpybWLiyZodJGBbI4aw.png)

![](https://cdn-images-1.medium.com/max/1024/1*sqfocZoPJQkTR5mWup0zuQ.png)
